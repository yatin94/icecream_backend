...
old_flavors = cur_old.execute("SELECT * FROM Flavors;").fetchall()
print(old_flavors)
...
for old_flavor in old_flavors:
    ...
    cur_new.execute(
        """
        INSERT INTO Flavors (id, flavor_name, is_deleted,is_activated,  is_sundae )
        VALUES (?, ?, ?, ?, ?);
        """,
        (old_flavor[1], old_flavor[0], old_flavor[2], old_flavor[3], 0)  # Map the correct indices
    )
...
new_flavors = cur_new.execute("SELECT * FROM Flavors;").fetchall()
